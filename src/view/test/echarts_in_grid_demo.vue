  ...
  <div class="screen">
    ...
    <div class="grid_container">
      <div v-for="(item, i) in layout" :class="`item transparent color-${i%6}`" v-bind:key="i"
        ...
        <echartsCompontents :echart-id="item.echartId" v-on:registeEcharts="registeEcharts"></echartsCompontents>
      </div>
    </div>
...
<script>
  ...
  export default {
    data(){
      return {
        layout: [
          {cols: 3, rows: 6, echartId: 'echart1'},
          {cols: 3, rows: 6, echartId: 'echart2'},
        ],
        echartsArray: [
